...
} from '../../../services/conversationsAction';
import Video from 'react-native-video';
import {
  conversationsCollection,
  seekerRequestCollection,
} from '../../../config/firestore';
import {setFormatAsPerGiftedChatArray} from '../../../services/generalAction';
...
class Chat extends React.Component {
  ...
  getData = () => {
    if (this.getType() === 'seeker') this.getDataFromSeeker();
    else this.getDataFromConversation();
  };
  ...
  onActionIndexPress = (index) => {
    if (index === 0) this.openGallery();
    else if (index === 1) this.openCamera();
  };
            ...
            <TouchableWithoutFeedback onPress={this.backPress}>
              <View style={{width: 40}}>
                <Icon
                  type={'Feather'}
                  name="chevron-left"
                  style={{color: theme.primaryColor, fontSize: 35}}
                />
              </View>
  ...
  renderToolbar = (props) => {
        ...
        <View
          ...
          <Text
            style={{
              fontSize: 13,
              fontWeight: '600',
            }}>{`You blocked ${name}.`}</Text>
          <Text
            style={{
              marginLeft: 5,
              fontSize: 13,
              fontWeight: '600',
              color: 'rgb(19,162,234)',
            }}>
            Delete chat.
          </Text>
        </View>
    ...
    return (
      <MessageInputToolBar
        theme={theme}
        {...props}
        replyItem={this.state.replyItem}
      />
    );
  };
        ...
        <TouchableWithoutFeedback onPress={this.onAttachmentPress}>
          <View
            ...
            <Icon
              type={'Feather'}
              name={'paperclip'}
              style={{color: theme.primaryColor, fontSize: 22}}
            />
          </View>
        </TouchableWithoutFeedback>
      </View>
  ...
  renderComposer = (props) => {
    return (
      <Composer
        {...props}
        textInputStyle={{justifyContent: 'center', paddingTop: 8}}
      />
    );
  };
  ...
  renderAccessory = (props) => {
    ...
    if (regex.isEmpty(replyItem)) return null;
  ...
  renderMessageVideo(props) {
    ...
    return (
      <View style={{backgroundColor: props.theme.pinkColor, borderRadius: 15}}>
        <Video
          ref={(r) => {
            this.player = r;
          }}
