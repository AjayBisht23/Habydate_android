...
import {updateNotificationStatus} from '../../../../services/notificationsAction';
...
class NotificationItem extends Component {
  ...
  renderItem = () => {
    ...
    if (notification_type === 'matches') {
      return (
        <View style={[styles.nameView]}>
          <Text style={[styles.likeText, {color: theme.secondaryColor}]}>
          ...
          <Text
            style={[
              styles.timeText,
              {color: theme.subSecondaryColor, marginTop: 8},
            ]}>
    ...
    let getSubTitle = ` Sent you a ${getSeekerTitle(
      seekerKey,
      ...
      <View style={[styles.nameView]}>
        ...
        <View
          ...
          <Text
            style={[
              styles.timeText,
              {color: theme.subSecondaryColor, marginLeft: 5},
            ]}>
            {address}
          </Text>
        </View>
        ...
        <Text style={[styles.timeText, {color: theme.subSecondaryColor}]}>
          {moment.unix(createdAt).local().fromNow(true)}
        </Text>
      </View>
    );
