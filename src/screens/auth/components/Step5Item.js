...
class Step5Item extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  onMaritalPress = (item) => {
    ...
    if (item.title === maritalStatus) this.setState({maritalStatus: ''});
    else
  ...
  renderMaritalItem = ({item}) => {
    ...
    let selected = false;
    if (maritalStatus === item.title) selected = true;
